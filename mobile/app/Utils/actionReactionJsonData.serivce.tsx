...
import {ServiceReactionInterface} from "../Interfaces/ModalData.interface";
import {ModalDataInterface, ServiceActionInterface} from "../Interfaces/ModalData.interface";
...
namespace actionReactionJsonDataService {
    ...
    export const getActionJsonData = async (serviceId: number): Promise<ServiceActionInterface[]> => {
        try {
            const response: Response = await requestApi(`${process.env.EXPO_PUBLIC_API_URL}/area/getActionsFromServiceId/${serviceId}`, "GET", null)
            if (!response.ok) {
    ...
    export const getReactionJsonData = async (actionId: number): Promise<ServiceReactionInterface[]> => {
        try {
            const response: Response = await requestApi(`${process.env.EXPO_PUBLIC_API_URL}/area/getReactionsFromActionId/${actionId}`, "GET", null)
            if (!response.ok) {
    ...
    export const getOauthIdsFromServiceId = async (serviceId: number): Promise<number[]> => {
        try {
            const response: Response = await requestApi(`${process.env.EXPO_PUBLIC_API_URL}/area/getOauthIdsFromServiceId/${serviceId}`, "GET", null)
            if (!response.ok) {
    ...
    export const getOauthIdsFromActionId = async (actionId: number): Promise<number[]> => {
        try {
            const response: Response = await requestApi(`${process.env.EXPO_PUBLIC_API_URL}/area/getOauthIdsFromActionId/${actionId}`, "GET", null)
            if (!response.ok) {
    ...
    export const getOauthIdsFromReactionId = async (reactionId: number): Promise<number[]> => {
        try {
            const response: Response = await requestApi(`${process.env.EXPO_PUBLIC_API_URL}/area/getOauthIdsFromReactionId/${reactionId}`, "GET", null)
            if (!response.ok) {
